...
const analysisCache = new Map<string, { summary: string; timestamp: number }>()
...
export async function analyzeData(data: any): Promise<string> {
  try {
    ...
    if (cachedAnalysis && Date.now() - cachedAnalysis.timestamp < CACHE_TTL) {
      ...
      return cachedAnalysis.summary
    }

    // Function to truncate data for analysis - more aggressive truncation
    const truncateData = (data: any): any => {
      if (Array.isArray(data)) {
        // If it's an array, limit to first 5 items (reduced from 10)
    ...
    // Get enhanced data statistics for better analysis
    ...
    const healthContext = isHealthData
      ? `This appears to be health data. Consider health indicators, public health implications, and include specific numerical findings.`
      : ""

    // Convert truncated data to a string representation for the LLM - limit size
    const dataString =
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        // Generate a summary using Groq with reduced tokens
        const { text } = await generateText({
          model: groq("llama-3.1-8b-instant"),
          prompt: `
            You are a data analyst assistant specializing in health data. Analyze the following data sample and provide a comprehensive summary.
            
            IMPORTANT: Your summary must include BOTH qualitative insights AND quantitative findings with specific numbers.
            ...
            SUMMARY (include both qualitative insights AND specific numerical findings):
          `,
          maxTokens: 600, // Increased from 400 to allow for more detailed analysis
        })

        // Cache the successful analysis
        analysisCache.set(cacheKey, { summary: text.trim(), timestamp: Date.now() })
    ...
    return "I've analyzed your data, but couldn't generate a detailed summary due to API rate limits. You can still ask specific questions about your data or request visualizations."
  }
}
...
function getEnhancedDataStatistics(data: any): string {
  try {
    ...
    return "Could not generate detailed statistics for this dataset."
  }
}
